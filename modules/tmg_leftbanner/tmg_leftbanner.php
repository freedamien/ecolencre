...
include_once(_PS_MODULE_DIR_.'tmg_leftbanner/LeftBanner.php');

class Tmg_LeftBanner extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('TMG - Left Banner', array(), 'Modules.LeftBanner');
        ...
        $this->templateFile = 'module:tmg_leftbanner/views/templates/hook/tmgleftbanner.tpl';
    }
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmgleftbanner` (
                `id_tmgleftbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_tmgleftbanner_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmgleftbanner_slides` (
              `id_tmgleftbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_tmgleftbanner_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmgleftbanner_slides_lang` (
              `id_tmgleftbanner_slides` int(10) unsigned NOT NULL,
              `id_lang` int(10) unsigned NOT NULL,
              `title` varchar(255) NOT NULL,
              `url` varchar(255) NOT NULL,
              `image` varchar(255) NOT NULL,
              PRIMARY KEY (`id_tmgleftbanner_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmgleftbanner`, `'._DB_PREFIX_.'tmgleftbanner_slides`, `'._DB_PREFIX_.'tmgleftbanner_slides_lang`;
        ');
    }
        ...
        if (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_slide')) {
            ...
            foreach ($languages as $language) {
                ...
                if (isset($_FILES['image_'.$language['id_lang']]) &&
                    ...
                    $temp_name = tempnam(_PS_TMGP_IMG_DIR_, 'PS');
                    $salt = sha1(microtime());
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'tmgleftbanner (id_tmgleftbanner_slides, id_shop)
            SELECT id_tmgleftbanner_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'tmgleftbanner
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'tmgleftbanner_slides` hss, `'._DB_PREFIX_.'tmgleftbanner` hs
            WHERE hss.`id_tmgleftbanner_slides` = hs.`id_tmgleftbanner_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_tmgleftbanner_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`image`
            FROM '._DB_PREFIX_.'tmgleftbanner hs
            LEFT JOIN '._DB_PREFIX_.'tmgleftbanner_slides hss ON (hs.id_tmgleftbanner_slides = hss.id_tmgleftbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'tmgleftbanner_slides_lang hssl ON (hss.id_tmgleftbanner_slides = hssl.id_tmgleftbanner_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'tmg_leftbanner/views/img/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'tmgleftbanner_slides hss ON (hs.id_tmgleftbanner_slides = hss.id_tmgleftbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'tmgleftbanner_slides_lang hssl ON (hss.id_tmgleftbanner_slides = hssl.id_tmgleftbanner_slides)
            WHERE hs.`id_tmgleftbanner_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_tmgleftbanner_slides` as id_slide
                FROM `'._DB_PREFIX_.'tmgleftbanner` hs
                WHERE hs.`id_tmgleftbanner_slides` = '.(int)$id_slide;
