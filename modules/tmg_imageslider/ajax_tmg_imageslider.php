...
include_once('tmg_imageslider.php');

$home_slider = new Tmg_ImageSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'tmghomeslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_tmghomeslider_slides` = '.(int)$id_slide
		);
